...
| Ordered anti-cancer regimen | PA should evaluate the coordinated regimen, not isolated drugs | Proposed `PlanDefinition` profile with `type = order-set` | Missing in mCODE | mCODE has treatment resources but no first-class anti-cancer regimen/order-set profile | Create `OncologyAntiCancerRegimenPlanDefinition`; include in CDS Hooks `draftOrders` |
